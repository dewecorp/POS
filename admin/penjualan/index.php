...
<form method="GET" class="flex flex-wrap gap-2 text-xs"><input name="q" value="<?=e($q)?>" placeholder="No transaksi / kasir" class="rounded-xl border px-3 py-2 flex-1"><input type="date" name="from" value="<?=e($from)?>" class="rounded-xl border px-2 py-2"><input type="date" name="to" value="<?=e($to)?>" class="rounded-xl border px-2 py-2"><select name="status" class="rounded-xl border px-2 py-2"><option value="">Semua status</option><option value="completed" <?=$status==='completed'?'selected':''?>>Completed</option><option value="cancelled" <?=$status==='cancelled'?'selected':''?>>Cancelled</option><option value="corrected" <?=$status==='corrected'?'selected':''?>>Corrected</option><option value="refunded" <?=$status==='refunded'?'selected':''?>>Refunded</option></select><button class="rounded-xl bg-emerald-600 px-4 py-2 text-white">Filter</button></form>
...
<a href="<?=APP_URL?>/kasir/cetak-struk.php?id=<?=$r['id']?>" target="_blank" title="Cetak Struk" class="inline-flex h-7 w-7 items-center justify-center rounded-lg border border-emerald-300 bg-emerald-50 text-emerald-600 hover:bg-emerald-100 transition"><svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4H7v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"/></svg></a>
