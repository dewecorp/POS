...
if($_SERVER['REQUEST_METHOD']==='POST'){
 ...
 $act=$_POST['act']??'';
 if($act==='create'){
  $code=trim($_POST['code']); $name=trim($_POST['name']); $phone=trim($_POST['phone']??''); $address=trim($_POST['address']??''); $type=$_POST['type']??'umum';
...
?>
<!DOCTYPE html><html lang="id"><head><title>Pelanggan • <?=e(APP_NAME)?></title><?php include __DIR__.'/../../components/head.php'; ?></head>
...
<form method="GET" class="flex gap-2"><input name="q" value="<?=e($q)?>" placeholder="Cari pelanggan" class="flex-1 rounded-xl border px-3 py-2 text-xs"><button class="rounded-xl border bg-white px-4 py-2 text-xs">Cari</button></form>
...
<form method="POST" class="space-y-2"><input type="hidden" name="_csrf" value="<?=e(csrf_token())?>"><input type="hidden" name="act" value="create"><input name="code" required placeholder="CUST004" class="w-full rounded-xl border px-3 py-2 text-xs"><input name="name" required placeholder="Nama" class="w-full rounded-xl border px-3 py-2 text-xs"><input name="phone" placeholder="HP" class="w-full rounded-xl border px-3 py-2 text-xs"><textarea name="address" placeholder="Alamat" class="w-full rounded-xl border px-3 py-2 text-xs"></textarea><select name="type" class="w-full rounded-xl border px-3 py-2 text-xs"><option value="umum">Umum</option><option value="member">Member</option><option value="grosir">Grosir</option></select><button class="w-full rounded-xl bg-emerald-600 py-2 text-xs text-white">Simpan</button></form></div></div></div>
