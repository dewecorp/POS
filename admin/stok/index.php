...
?>
<!DOCTYPE html><html lang="id"><head><title>Stok • <?=e(APP_NAME)?></title><?php include __DIR__.'/../../components/head.php'; ?></head>
...
<form method="GET" class="flex gap-2 text-xs"><input name="q" value="<?=e($q)?>" placeholder="Cari produk" class="flex-1 rounded-xl border px-3 py-2"><select name="type" class="rounded-xl border px-2 py-2"><option value="">Semua tipe</option><option value="PURCHASE" <?=$type=='PURCHASE'?'selected':''?>>PURCHASE</option><option value="SALE" <?=$type=='SALE'?'selected':''?>>SALE</option><option value="ADJUSTMENT" <?=$type=='ADJUSTMENT'?'selected':''?>>ADJUSTMENT</option><option value="STOCK_OPNAME" <?=$type=='STOCK_OPNAME'?'selected':''?>>OPNAME</option><option value="CORRECTION" <?=$type=='CORRECTION'?'selected':''?>>CORRECTION</option></select><button class="rounded-xl bg-slate-800 px-4 py-2 text-white">Filter</button></form>
