...
<div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><h2 class="text-lg font-semibold">Master Produk</h2><div class="flex gap-2"><button data-modal-toggle="#modalAdd" class="rounded-full bg-emerald-600 px-4 py-1.5 text-xs text-white">+ Tambah Produk</button><a href="<?=APP_URL?>/api/products.php?export=csv" class="rounded-full border bg-white px-4 py-1.5 text-xs">Export CSV</a></div></div>
<form method="GET" class="flex flex-wrap gap-2"><input name="q" value="<?=e($q)?>" placeholder="Cari SKU/barcode/nama" class="flex-1 min-w-[180px] rounded-xl border px-3 py-2 text-xs"><select name="cat" class="rounded-xl border px-3 py-2 text-xs"><option value="">Semua kategori</option><?php foreach($cats as $c):?><option value="<?=$c['id']?>" <?=$catF==$c['id']?'selected':''?>><?=e($c['name'])?></option><?php endforeach;?></select><button class="rounded-xl border bg-white px-4 py-2 text-xs">Cari</button></form>
